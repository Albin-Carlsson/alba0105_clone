...
#define Success(link, val) (option_list_t) {.success=true, .ptr=link, .value = val};
...
static option_list_t find_previous_link(ioopm_list_t* list, int index) {
    ...
    do {
        if (size == index)
        {
            return Success(current, current->value);
        }
        size++;
...
option_list_t ioopm_linked_list_insert(ioopm_list_t* list, int index, elem_t value) {
    ...
    list->size = list->size + 1;
    return Success(new_link, new_link->value);
}
...
option_list_t ioopm_linked_list_remove(ioopm_list_t* list, int index) {
    ...
    previous->next=next;
    list->size = list->size -1;
    ioopm_link_t * ptr_ignore = NULL;
    return Success(ptr_ignore, removed_value);
}
...
option_list_t ioopm_linked_list_get(ioopm_list_t* list, int index) {
    ...
    return Success(to_get, to_get->value);
}
